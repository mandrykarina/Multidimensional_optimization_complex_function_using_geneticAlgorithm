...
| `docs/DEFENSE.md` | Короткие пояснения для защиты |
...
Можно создать отдельный репозиторий или поместить папку лабораторной в общий репозиторий. Добавляйте исходники, `data`, `results`, `docs`, README и отчёт. `.venv` и кеши уже исключены через `.gitignore`. Картинки подключены относительными путями и отображаются на GitHub; схемы Mermaid находятся в отчёте.

```bash
git init
git add .
git commit -m "Add lab 1: variant 16, group 507"
```

Репозиторий и remote выбираются вами; команды загрузки не выполнялись. Оформление использует графики реально выполненных экспериментов, поэтому все иллюстрации связаны с результатами работы.
